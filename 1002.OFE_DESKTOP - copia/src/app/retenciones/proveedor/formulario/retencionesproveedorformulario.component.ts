...
var oretencionesProveedorFormularioComponent: RetencionesProveedorFormularioComponent, dtArticulos;
@Component({
  moduleId: module.id,
  selector: 'Retencionesproveedorformulario-cmp',
  templateUrl: 'Retencionesproveedorformulario.component.html',
  providers: [retencionesService, MasterService]
})

export class RetencionesProveedorFormularioComponent implements OnInit, AfterViewInit {
  ...
  print(event): void {
    ...
    oretencionesProveedorFormularioComponent.item.moneda_txt = $("#moneda option:selected").text();
    oretencionesProveedorFormularioComponent.item.estado = $("#estadoProveedor option:selected").text();
    setTimeout(function () {
  ...
  ngOnInit() {
    ...
    this.util.listMonedas(function (data: ComboItem[]) {
      ...
      oretencionesProveedorFormularioComponent.listMonedaCombo = data;
    });
  ...
  ngAfterViewInit() {
    ...
    dtArticulos = $('#dtArticulos').DataTable({
      footerCallback: function (row, data, start, end, display) {
        ...
        oretencionesProveedorFormularioComponent.item.total = DatatableFunctions.FormatNumber(total);

        $(api.column(6).footer()).html(
          oretencionesProveedorFormularioComponent.item.total
        );
      },
      "order": [[1, "asc"]],
      "ajax": function (data, callback, settings) {
        let result = {
          data: oretencionesProveedorFormularioComponent.item.productos
